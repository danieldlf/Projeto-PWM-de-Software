...
#define VOLT_THRESHOLD 128
#define LDR_THRESHOLD 128
...
void MAX7219_Init(void) {
    MAX7219_SendData(0x09, 0x00); // No decode
    MAX7219_SendData(0x0A, 0x00); // Brightness
    MAX7219_SendData(0x0B, 0x07); // Scan limit
    MAX7219_SendData(0x0C, 0x01); // Enable
    MAX7219_SendData(0x0F, 0x00); // No test mode
    for (int i = 1; i <= 8; i++) {
        MAX7219_SendData(i, 0x00); // Clear display
    }
}

void MAX7219_DisplayPattern(const uint8_t *char_pattern) {
    for (int i = 0; i < 8; i++) {
        MAX7219_SendData(i + 1, char_pattern[i]);
    }
}

void MAX7219_AlternatePattern(const uint8_t *main_char, uint8_t value, uint16_t delay_ms) {
    const uint8_t *sign_pattern = (value < 128) ? char_minus : char_plus;
    for (int i = 0; i < 10; i++) {
        Display_Char(main_char);
        HAL_Delay(delay_ms);
        Display_Char(sign_pattern);
        HAL_Delay(delay_ms);
    }
    MAX7219_Init(); // Clear display after alternation
}

char *Clean_Buffer(char *command) {
...
uint8_t PCF8591_ReadChannel(uint8_t channel) {
    uint8_t command_ain;
    uint8_t data;
...
void Process_Sensor(const char *label, const uint8_t *char_pattern, uint8_t channel, uint8_t threshold) {
    uint8_t value = Read_PCF8591(channel);
    char msg[20];
    sprintf(msg, "%s: %d\n", label, value);
    HAL_UART_Transmit(&huart3, (uint8_t *)msg, strlen(msg), HAL_MAX_DELAY);
    Display_Alternating(char_pattern, value, DISPLAY_DELAY_MS);
}
...
void Process_Command(char *command) {
    ...
    if (strcmp(command, "Temp") == 0) {
        Process_Sensor_Command("Temperature", char_T, 0, TEMP_THRESHOLD);
    } else if (strcmp(command, "Volt") == 0) {
        Process_Sensor_Command("Voltage", char_V, 1, VOLT_THRESHOLD);
    } else if (strcmp(command, "LDR") == 0) {
        Process_Sensor_Command("Luminosity", char_L, 3, LDR_THRESHOLD);
    }
}
